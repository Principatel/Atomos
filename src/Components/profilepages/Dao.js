...
function Daos() {
  ...
  useEffect(() => {
    ...
    if (data && data.Poaps && data.Poaps.Poap) {
      const poapImages = data.Poaps.Poap.map(
        (poap) => poap.poapEvent.contentValue.image.small
      );
      const nftImages = data.TokenBalances.TokenBalance.map(
        (TokenBalance) => TokenBalance.tokenNfts.contentValue.image?.small
